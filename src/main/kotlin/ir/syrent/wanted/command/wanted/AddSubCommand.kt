...
class AddSubCommand : SubCommand("add", "wanted.command.add", false) {
    ...
    override fun onExecute(sender: CommandSender, args: List<String>) {
        Settings.load()
        if (args.size != 2) {
            sender.sendMessage(Message.WANTED_ADD_USAGE)
            return
        }
        ...
        if (!WantedManager.addWanted(target.get().uuid, args[1])) {
            sender.sendMessage(Message.VALID_PARAMS, TextReplacement("argument", args[1]))
            return
        }

        sender.sendMessage(Message.WANTED_ADD_USE, TextReplacement("player", target.get().username), TextReplacement("count", args[1]))
    }
