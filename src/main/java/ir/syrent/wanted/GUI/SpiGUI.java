...
import ir.syrent.wanted.GUI.menu.SGMenuListener;
import ir.syrent.wanted.GUI.menu.SGOpenMenu;
import ir.syrent.wanted.GUI.pagination.SGPaginationButtonBuilder;
        ...
        switch (type) {
            ...
            case CURRENT_BUTTON:
                return new SGButton(new ItemBuilder(Material.NAME_TAG)
                        .name("&7Page &b" + (inventory.getCurrentPage() + 1) + " &7of &b" + inventory.getMaxPage())
                        .lore(
                                "&7You are currently viewing",
                                "&7page &b" + (inventory.getCurrentPage() + 1) + "&7."
                        ).build()
